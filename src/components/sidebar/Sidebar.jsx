      ...
      >
        <div className='col-span-2 md:py-10'>
          <Link to={'/'} onClick={() => window.scroll(0, 0)}>
            <h1 className='border-white stroke-white text-xl font-bold text-main md:pl-5'>HealthifyMe</h1>
          </Link>
        </div>
        <div
          className={`absolute ${
        ...
        >
          {dataAplikasi &&
            dataAplikasi.map((element, index) => {
              const isActive = location.pathname === element.path; // Cek apakah path saat ini sama dengan path item
              return (
                <Link to={element.path} onClick={() => window.scroll(0, 0)} key={index}>
                  <div className={`group flex items-center gap-x-4 py-4 hover:bg-main md:px-5 md:pl-4 ${isActive ? 'bg-blue-500 text-white' : ''}`}>
                    <img src={element.icon} alt='Kalkulator Kalori' className='mx-auto w-20 md:mx-0 md:w-10' />
                    <h2 className={`hidden text-lg font-semibold group-hover:text-white md:block ${isActive ? 'text-white' : ''}`}>{element.title}</h2>
                  </div>
                </Link>
              );
            })}
